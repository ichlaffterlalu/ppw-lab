...
class Lab4UnitTest(TestCase):
	...
	def test_lab4_post_fail_message_empty(self):
		response = client_main.post('/lab-4/add_message', {'name': 'Anonymous', 'email': 'a@a.com', 'message': ''})
		self.assertEqual(response.status_code, 302)

	def test_lab4_post_fail_invalid_email(self):
		response = client_main.post('/lab-4/add_message', {'name': 'Anonymous', 'email': 'A', 'message': 'Saya pergi ke pasar.'})
		self.assertEqual(response.status_code, 302)
	...
	def test_lab4_post_success_and_render_the_result(self):
		anonymous = 'Anonymous'
		message = 'HaiHai'
		response = client_main.post('/lab-4/add_message', {'name': '', 'email': '', 'message': message})
		self.assertEqual(response.status_code, 200)
		html_response = response.content.decode('utf8')
	...
	def test_lab_4_table_url_exist(self):
		response = client_main.get('/lab-4/result_table')
		self.assertEqual(response.status_code, 200)
	...
	def test_lab_4_showing_all_messages(self):
		...
		post_data_budi = client_main.post('/lab-4/add_message', data_budi)
		...
		post_data_anonymous = client_main.post('/lab-4/add_message', data_anonymous)
